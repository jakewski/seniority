$(document).ready(function() {
  $("#prof").click(function() {
    ...
    // })
    axios.get("/auth/me", (req, res, next) => res.data).then(me => {
      axios.get(`/helper/available/min/${me.location[0]}/${me.location[1]}`).then(helper => {
        if (!helper) {
          //nobody in distance
        } else {
          //send request to closest person
          res.sendStatus(200);
        }
      });
    })
...
$(".banner-slider").backstretch(["images/bg-1.jpg", "images/bg-2.jpg"], {
  fade: 750,
  duration: 3000
});
